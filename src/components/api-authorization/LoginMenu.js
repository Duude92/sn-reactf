...
export class LoginMenu extends Component {
  ...
  render() {
    ...
    if (!isAuthenticated) {
      ...
      const profilePath = `${this.state.userName}`;
  ...
  anonymousView(registerPath, loginPath) {
    return (<Fragment>
      <NavItem>
        <Button component={LinkRef} sx={{ display: 'block' }} to={registerPath}  >Register</Button>

        {/* <NavLink tag={Link} className="text-dark navlink" to={registerPath}>Register</NavLink> */}
      </NavItem>
      <NavItem>
        <Button component={LinkRef} sx={{ display: 'block' }} to={loginPath}  >Login</Button>
